...
const tr = (locale: Locale, text: CopyText) => pickByLocale(locale, text);
  ...
  return (
    <div className="space-y-4">
      <div className="px-2 pt-1">
        <p className="font-serif text-[30px] text-[#1A1A1A]">{tr(locale, { zh: "三月十五", en: "March 15", hr: "15. ožujka" })}</p>
        <p className="mt-1 text-sm text-[#8E8E93]">{tr(locale, { zh: "乙巳年 · 二月十六 · 周六", en: "Saturday · Lunar Feb 16", hr: "Subota · Lunarni 16. veljače" })}</p>
      </div>
      ...
      <div className="rounded-[24px] border border-black/5 bg-white px-5 py-5 shadow-[0_8px_30px_-24px_rgba(26,26,26,0.3)]">
        <div className="flex items-center justify-between text-xs uppercase tracking-[0.3em] text-[#8E8E93]">
          <span>{tr(locale, { zh: "诗词", en: "Poetry", hr: "Poezija" })}</span>
        ...
        <div className="px-2 py-7 text-center">
          ...
          <p className="font-serif text-sm text-[#8E8E93]">{tr(locale, { zh: "苏轼 · 《浣溪沙》", en: "Su Shi · Huan Xi Sha", hr: "Su Shi · Huan Xi Sha" })}</p>
        </div>
        <div className="rounded-[16px] border border-[#E8E8E4] bg-[#F5F5F0] p-4">
          <p className="text-center text-[11px] text-[#A0A0A0]">{tr(locale, { zh: "墨水屏预览效果", en: "E-ink preview", hr: "E-ink pregled" })}</p>
        ...
        <div className="mt-5 flex items-center justify-around border-t border-[#F0F0F0] pt-4 text-xs text-[#8E8E93]">
          <span className="inline-flex items-center gap-1.5"><RefreshCcw size={14} strokeWidth={1.8} />{tr(locale, { zh: "换一首", en: "Refresh", hr: "Osvježi" })}</span>
          <span className="inline-flex items-center gap-1.5"><Share2 size={14} strokeWidth={1.8} />{tr(locale, { zh: "分享", en: "Share", hr: "Podijeli" })}</span>
          <span className="inline-flex items-center gap-1.5"><Send size={14} strokeWidth={1.8} />{tr(locale, { zh: "推送到设备", en: "To device", hr: "Na uređaj" })}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between rounded-[18px] border border-black/5 bg-white px-4 py-3 shadow-[0_8px_20px_-24px_rgba(26,26,26,0.45)]">
        <div className="inline-flex items-center gap-2 text-sm text-[#1A1A1A]">
          ...
          <span>{tr(locale, { zh: "晴 18°C", en: "Sunny 18°C", hr: "Sunčano 18°C" })}</span>
        </div>
        <span className="text-xs text-[#8E8E93]">{tr(locale, { zh: "适合出行", en: "Good for a walk", hr: "Dobro za šetnju" })}</span>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      {browseSegment === "widgets" ? (
          ...
          {widgetIcons.map(({ name, icon: Icon }, index) => (
            <div key={name} className="rounded-[18px] border border-black/5 bg-white px-2 py-4 text-center shadow-[0_8px_18px_-24px_rgba(26,26,26,0.5)]">
              ...
              <p className={`mt-1 text-[10px] ${index % 2 === 0 ? "text-[#34C759]" : "text-[#8E8E93]"}`}>
                {index % 2 === 0 ? tr(locale, { zh: "已启用", en: "Enabled", hr: "Omogućeno" }) : tr(locale, { zh: "可预览", en: "Preview", hr: "Pregled" })}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-3">
          <p className="px-1 text-xs font-medium text-[#8E8E93]">{tr(locale, { zh: "今天", en: "Today", hr: "Danas" })}</p>
          {current.items.map((item) => (
            <div key={`${browseSegment}-${item.title}`} className="flex gap-3 rounded-[18px] border border-black/5 bg-white p-3 shadow-[0_8px_18px_-24px_rgba(26,26,26,0.5)]">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[14px] border border-[#E8E8E4] bg-[#F5F5F0] text-[11px] text-[#8E8E93]">
                {tr(locale, { zh: "预览", en: "Preview", hr: "Pregled" })}
              </div>
...
function CreatePhone({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="space-y-4">
      <div className="px-2 pt-1">
        <p className="font-serif text-[30px] font-semibold text-[#1A1A1A]">{tr(locale, { zh: "创作", en: "Create", hr: "Izrada" })}</p>
        <p className="mt-1 text-sm text-[#8E8E93]">{tr(locale, { zh: "模板、AI 生成、实时预览", en: "Templates, AI, and live preview", hr: "Predlošci, AI i pregled uživo" })}</p>
      </div>
      ...
      <div className="rounded-[22px] border border-black/5 bg-white p-4 shadow-[0_8px_18px_-24px_rgba(26,26,26,0.5)]">
        ...
        <div className="mt-4 grid gap-3">
          <div className="rounded-[14px] bg-[#F5F5F0] px-4 py-3 text-sm text-[#8E8E93]">
            {tr(locale, { zh: "Prompt: 生成适合深度工作的平静日签。", en: "Prompt: write a calm daily note for deep work.", hr: "Prompt: napiši mirnu dnevnu bilješku za duboki rad." })}
          </div>
          <div className="rounded-[14px] bg-[#F5F5F0] px-4 py-3 text-sm text-[#8E8E93]">
            {tr(locale, { zh: "每 60 分钟刷新，衬线标题，紧凑页脚。", en: "Refresh every 60 minutes, serif headline, compact footer.", hr: "Osvježi svakih 60 minuta, serifni naslov, kompaktan footer." })}
          </div>
          <div className="rounded-[16px] border border-[#E8E8E4] bg-[#FAFAF8] p-4">
            <p className="text-center text-[11px] text-[#A0A0A0]">{tr(locale, { zh: "实时预览", en: "Live preview", hr: "Pregled uživo" })}</p>
            <div className="mt-2 rounded-[12px] bg-white px-4 py-5 text-center font-serif text-[17px] leading-7 text-[#1A1A1A]">
              {tr(locale, { zh: "守住一件事。", en: "Stay with one thing.", hr: "Drži se jedne stvari." })}
            </div>
          </div>
        </div>
      </div>
...
function DevicePhone({ locale }: { locale: Locale }) {
  const rows = [
    ...
    { label: tr(locale, { zh: "固件升级", en: "Firmware", hr: "Firmware" }), value: "v2.1.0" },
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between px-2 pt-1">
        <div>
          <p className="font-serif text-[30px] font-semibold text-[#1A1A1A]">{tr(locale, { zh: "书房墨鱼", en: "Study Ink", hr: "Study Ink" })}</p>
          <p className="mt-1 text-sm text-[#8E8E93]">{tr(locale, { zh: "设备仪表盘与 BLE 配网", en: "Device dashboard and BLE setup", hr: "Nadzorna ploča uređaja i BLE postavljanje" })}</p>
        </div>
        <span className="pt-2 text-sm text-[#007AFF]">{tr(locale, { zh: "返回", en: "Back", hr: "Natrag" })}</span>
      </div>
      ...
      <div className="grid grid-cols-3 rounded-[22px] border border-black/5 bg-white px-4 py-5 text-center shadow-[0_8px_18px_-24px_rgba(26,26,26,0.5)]">
        <div>
          ...
          <p className="mt-1 text-[11px] text-[#8E8E93]">{tr(locale, { zh: "锂电池", en: "Lithium Battery", hr: "Litijska baterija" })}</p>
        </div>
        <div className="border-x border-[#E5E5EA]">
          <p className="text-2xl font-light text-[#34C759]">●</p>
          <p className="mt-1 text-[11px] text-[#8E8E93]">{tr(locale, { zh: "在线", en: "Online", hr: "Online" })}</p>
        </div>
        <div>
      ...
      <div className="rounded-[22px] border border-black/5 bg-white p-4 shadow-[0_8px_18px_-24px_rgba(26,26,26,0.5)]">
        <p className="text-sm font-medium text-[#1A1A1A]">{tr(locale, { zh: "当前显示内容", en: "Current display", hr: "Trenutni prikaz" })}</p>
        ...
        <button type="button" className="mt-4 w-full rounded-full bg-[#007AFF] px-4 py-2.5 text-sm font-medium text-white">
          {tr(locale, { zh: "立即刷新", en: "Refresh now", hr: "Osvježi sada" })}
        </button>
      </div>
      ...
      <div className="space-y-px overflow-hidden rounded-[20px] border border-black/5 bg-white shadow-[0_8px_18px_-24px_rgba(26,26,26,0.5)]">
        {rows.map((row) => (
          <div key={row.label} className="flex items-center justify-between px-4 py-3.5">
            ...
            <span className={`text-sm ${row.label === tr(locale, { zh: "固件升级", en: "Firmware", hr: "Firmware" }) ? "text-[#34C759]" : "text-[#8E8E93]"}`}>{row.value} &rsaquo;</span>
          </div>
        ))}
      </div>
    </div>
...
function MePhone({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="space-y-4">
      <div className="px-2 pt-1">
        <p className="font-serif text-[30px] font-semibold text-[#1A1A1A]">{tr(locale, { zh: "我的", en: "Me", hr: "Ja" })}</p>
        <p className="mt-1 text-sm text-[#8E8E93]">{tr(locale, { zh: "账号、偏好与首次引导", en: "Account, preferences, and onboarding", hr: "Račun, postavke i onboarding" })}</p>
      </div>
      ...
      <div className="rounded-[24px] border border-black/5 bg-white p-5 shadow-[0_8px_18px_-24px_rgba(26,26,26,0.5)]">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[#8E8E93]">{tr(locale, { zh: "慢信息收藏者", en: "Slow information collector", hr: "Kolekcionar sporih informacija" })}</p>
          </div>
        </div>
      </div>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10 pt-14">
        <div className="max-w-3xl">
          <span className="inline-flex rounded-full border border-ink/10 bg-white/80 px-4 py-1.5 text-xs uppercase tracking-[0.28em] text-ink-light">
            {tr(locale, { zh: "移动端原型实现", en: "Mobile App Prototype", hr: "Prototip mobilne aplikacije" })}
          </span>
          ...
          <p className="mt-5 max-w-2xl text-base leading-8 text-ink-light md:text-lg">
            {tr(
              locale,
              {
                zh: "这个页面把 `docs/mobile-app-design.md` 里的信息组织成真实界面：中心按钮 5-Tab、22 个页面映射、Lucide 墨感图标、米白纸张色系、后端扩展端点和四阶段实施路线图。",
                en: "This page turns `docs/mobile-app-design.md` into a concrete experience: a centered 5-tab shell, 22 mapped screens, Lucide ink-stroke iconography, iOS-toned visuals, backend API expansion, and a phased delivery plan.",
                hr: "Ova stranica pretvara `docs/mobile-app-design.md` u konkretno iskustvo: centralni 5-tab shell, 22 mapirana ekrana, Lucide ikonografiju, iOS tonalitet, backend proširenja i fazni plan isporuke.",
              },
            )}
          </p>
        </div>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {[
            tr(locale, { zh: "12 章规格文档", en: "12 spec chapters", hr: "12 poglavlja specifikacije" }),
            tr(locale, { zh: "22 个页面映射", en: "22 mapped screens", hr: "22 mapirana ekrana" }),
            tr(locale, { zh: "中心按钮 5-Tab", en: "Center-button 5-tab", hr: "Središnji 5-tab gumb" }),
            tr(locale, { zh: "Lucide 1.8px 图标", en: "Lucide 1.8px icons", hr: "Lucide 1.8px ikone" }),
            tr(locale, { zh: "6 个后端扩展端点", en: "6 backend extensions", hr: "6 backend proširenja" }),
            tr(locale, { zh: "4 个实施阶段", en: "4 delivery phases", hr: "4 faze isporuke" }),
          ].map((item) => (
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link href={withLocalePath(locale, "/docs")}>
            <Button className="rounded-full px-6">{tr(locale, { zh: "查看文档中心", en: "Open Docs", hr: "Otvori dokumentaciju" })}</Button>
          </Link>
          <Link href={withLocalePath(locale, "/config")}>
            <Button variant="outline" className="rounded-full px-6">{tr(locale, { zh: "查看设备配置", en: "Open Device Config", hr: "Otvori konfiguraciju uređaja" })}</Button>
          </Link>
        </div>
      </section>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              <div className="space-y-4">
                <div className="rounded-[24px] border border-ink/10 bg-[#fafaf8] p-5">
                  <h3 className="font-serif text-xl text-ink">{tr(locale, { zh: "页面栈", en: "Screen stack", hr: "Stack ekrana" })}</h3>
                ...
                <div className="rounded-[24px] border border-ink/10 bg-white p-5">
                  <h3 className="font-serif text-xl text-ink">{tr(locale, { zh: "交互说明", en: "Interaction notes", hr: "Napomene o interakciji" })}</h3>
                  <div className="mt-4 grid gap-3">
              ...
              <div className="space-y-4">
                <div className="rounded-[24px] border border-ink/10 bg-[#1A1A1A] p-5 text-white">
                  <h3 className="font-serif text-xl">{tr(locale, { zh: "API 映射", en: "API mapping", hr: "API mapiranje" })}</h3>
                  <div className="mt-4 space-y-3">
                        ...
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {api.isNew ? (
                            <span className="rounded-full bg-[#34C759]/20 px-2 py-0.5 text-[11px] text-[#9df3b2]">
                              {tr(locale, { zh: "新增", en: "New", hr: "Novo" })}
                            </span>
                          ) : null}
                        </div>
                ...
                <div className="rounded-[24px] border border-ink/10 bg-[#fafaf8] p-5">
                  <h3 className="font-serif text-xl text-ink">{tr(locale, { zh: "导航策略", en: "Tab rationale", hr: "Logika tabova" })}</h3>
                  <div className="mt-4 space-y-3 text-sm leading-6 text-ink-light">
                    <p>{locale !== "zh" ? "The centered create button turns UGC into a primary action without pushing device management out of the main shell." : "中间凸起的创作按钮让 UGC 成为一级入口，同时不挤压设备管理和首页沉浸阅读。"} </p>
                    <p>{locale !== "zh" ? "All tabs keep serif labels and 1.8px Lucide strokes to match the e-ink aesthetic." : "所有 Tab 保持衬线标签与 1.8px Lucide 描边，和墨水屏气质保持一致。"} </p>
                    <p>{locale !== "zh" ? "The shell is intentionally calm: fewer loud colors, no emoji, and a paper-first background." : "整体视觉故意克制：少量高饱和色、无 emoji、纸张感背景优先。"} </p>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
            <Card className="rounded-[32px] border-[#e8e3d8] bg-white/90">
              <CardHeader>
                <CardTitle className="font-serif text-2xl">{locale !== "zh" ? "Screen matrix" : "页面矩阵"}</CardTitle>
                <CardDescription className="leading-7">
                  {locale !== "zh" ? "22 mapped surfaces grouped by primary tab and shell role." : "按主 Tab 和壳层角色整理的 22 个页面，用来承接设计文档里的功能清单。"}
                </CardDescription>
              </CardHeader>
            ...
            <Card className="rounded-[32px] border-[#e8e3d8] bg-white/90">
              <CardHeader>
                <CardTitle className="font-serif text-2xl">{locale !== "zh" ? "Design language" : "设计语言"}</CardTitle>
                <CardDescription className="leading-7">
                  {locale !== "zh" ? "A zen palette, Lucide icons, serif labels, and iOS-like structural rhythm." : "禅意简约色调、Lucide 图标、衬线标签和 iOS 式信息节奏，共同构成移动端视觉语言。"}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {[
                    { icon: Feather, title: locale !== "zh" ? "Ink-stroke icons" : "墨感图标", body: locale !== "zh" ? "Lucide, 1.8px stroke, no emoji." : "Lucide 统一图标，1.8px 描边，无 emoji。" },
                    { icon: Palette, title: locale !== "zh" ? "Paper-first surfaces" : "纸张质感", body: locale !== "zh" ? "Paper white, light surface contrast, restrained blue accents." : "米白纸张、浅表面对比、克制的 iOS 蓝点缀。" },
                    { icon: BookOpen, title: locale !== "zh" ? "Serif typography" : "衬线排版", body: locale !== "zh" ? "Noto Serif SC for content and tab labels, SF-style UI for metadata." : "内容与 Tab 标签偏衬线，元信息保持系统字体感。" },
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
            <Card className="rounded-[32px] border-[#e8e3d8] bg-white/90">
              <CardHeader>
                <CardTitle className="font-serif text-2xl">{locale !== "zh" ? "Backend expansion" : "后端扩展计划"}</CardTitle>
                <CardDescription className="leading-7">
                  {locale !== "zh" ? "Six focused endpoints cover push, preferences, structured content, and widget payloads." : "围绕推送、偏好、结构化内容和 widget 数据新增 6 个端点，尽量复用现有后端能力。"}
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-4">
                ...
                <div className="rounded-[22px] border border-dashed border-ink/15 bg-white p-4 text-sm leading-7 text-ink-light">
                  <strong className="text-ink">{locale !== "zh" ? "Database additions:" : "数据库扩展："}</strong>{" "}
                  {locale !== "zh"
            ...
            <Card className="rounded-[32px] border-[#e8e3d8] bg-white/90">
              <CardHeader>
                <CardTitle className="font-serif text-2xl">{locale !== "zh" ? "Delivery roadmap" : "实施路线图"}</CardTitle>
                <CardDescription className="leading-7">
                  {locale !== "zh" ? "Four phases move from Expo MVP to shipping polish without overloading the first release." : "用四个阶段把 Expo MVP、设备能力、移动端特性和发布打磨拆开，避免首版范围失控。"}
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-4">
