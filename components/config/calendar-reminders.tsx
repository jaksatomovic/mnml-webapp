...
const MONTH_NAMES_HR = ["Sij", "Velj", "Ožu", "Tra", "Svi", "Lip", "Srp", "Kol", "Ruj", "Lis", "Stu", "Pro"];
...
export function CalendarReminders({ reminders, onChange, tr }: CalendarRemindersProps) {
  ...
  const formatKey = (key: string) => {
    ...
    if (localeKind === "en") return `${MONTH_NAMES_EN[m - 1]} ${d}`;
    if (localeKind === "hr") return `${d}. ${MONTH_NAMES_HR[m - 1]}`;
    return `${m}月${d}日`;
  };
    ...
    <div className="border border-ink/10 rounded-md p-3 space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {Array.from({ length: 12 }, (_, i) => (
            <option key={i + 1} value={i + 1}>
              {localeKind === "en" ? MONTH_NAMES_EN[i] : localeKind === "hr" ? MONTH_NAMES_HR[i] : `${i + 1}月`}
            </option>
          ))}
        </select>
        <select
        ...
        <input
          ...
          placeholder={tr("提醒内容", "Reminder", "Podsjetnik")}
          maxLength={20}
