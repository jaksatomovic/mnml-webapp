...
const WEEKDAYS_HR = ["Pon", "Uto", "Sri", "Čet", "Pet"];
...
export function TimetableEditor({ data, onChange, tr }: TimetableEditorProps) {
  ...
  const localeKind = tr("zh", "en", "hr");
  const weekdays = localeKind === "zh" ? WEEKDAYS_ZH : localeKind === "hr" ? WEEKDAYS_HR : WEEKDAYS_EN;
  const templateType = useMemo(() => detectTemplate(data), [data]);

  const getTemplate = useCallback((t: TemplateType) => {
    if (t === "k12") return localeKind === "zh" ? { ...TEMPLATE_K12_ZH } : { ...TEMPLATE_K12_EN };
    return localeKind === "zh" ? { ...TEMPLATE_UNIVERSITY_ZH } : { ...TEMPLATE_UNIVERSITY_EN };
  }, [localeKind]);
  ...
  const addPeriod = useCallback(() => {
    ...
    const label = templateType === "k12" ? (localeKind === "zh" ? `第${n}节` : `P${n}`) : `${8 + (n - 1) * 2}:00`;
    onChange({ ...data, periods: [...data.periods, label] });
  }, [data, onChange, templateType, localeKind]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        ...
        >
          {tr("中小学课表", "K-12", "Osnovna i srednja škola")}
        </button>
      </div>
                    ...
                    >
                      {isEditing ? (
                        <input
                          ...
                          placeholder={isUniv ? tr("课名/教室", "Name/Room", "Predmet/Učionica") : tr("课名", "Name", "Predmet")}
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" size="sm" onClick={addPeriod} className="h-7 px-2 text-xs">
          ...
          {tr("添加节次", "Add Period", "Dodaj termin")}
        </Button>
        <Button
          variant="outline"
          size="sm"
